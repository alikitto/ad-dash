...
export default function Tables() {
  ...
  const lastUpdatedLabel = useMemo(() => {
    ...
    const now = new Date(); const mins = Math.floor((now - lastUpdated)/60000); const hrs = Math.floor(mins/60);
    if (mins < 1) return "now"; if (mins === 1) return "1 min ago"; if (mins < 60) return `${mins} mins ago`;
    if (hrs === 1) return "1 hr ago"; return `${hrs} hrs ago`;
  }, [lastUpdated, tick]);

  const fetchData = useCallback(async () => {
    setLoading(true); setError(null);
    try {
      const res = await fetch(`${BACKEND}/api/adsets?date_preset=${datePreset}`);
  ...
  const handleStatusChange = async (adsetId, newStatus) => {
    ...
    try {
      const res = await fetch(`${BACKEND}/api/adsets/${adsetId}/update-status`, {
        method:"POST", headers:{ "Content-Type":"application/json" }, body:JSON.stringify({ status:newStatus })
      });
      if (!res.ok) throw new Error((await res.json().catch(()=>({}))).detail || "Failed to update status");
  ...
  const processedAdsets = useMemo(() => {
    ...
    if (selectedAccount !== "all") f = f.filter(a => a.account_name === selectedAccount);
    if (objectiveFilter !== "all") f = f.filter(a => a.objective === objectiveFilter);
    f.sort((a,b)=> a[sortConfig.key] < b[sortConfig.key] ? (sortConfig.direction==="ascending"?-1:1)
        ...
        <CardBody>
          <Box overflowX="auto" sx={{
            ...
            <Table variant="simple" color="#fff">
              <Thead>
                <Tr my=".8rem" ps="0px">
                  <Th color="white" position="sticky" left="0" zIndex="1" bg="#2a406e">Account / Campaign / Ad Set</Th>
