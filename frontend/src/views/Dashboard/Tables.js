...
import {
  ...
  useToast, HStack, Icon, IconButton, Button, Spacer
...
function Tables() {
  ...
  const SortableTh = ({ children, sortKey }) => (
    <Th cursor="pointer" onClick={() => requestSort(sortKey)} color="gray.200">
      <Flex align="center">{children}{sortConfig.key === sortKey && <Icon as={sortConfig.direction === "ascending" ? TriangleUpIcon : TriangleDownIcon} w={3} h={3} ml={2} />}</Flex>
    </Th>
  );
        ...
        <CardHeader>
          <Flex direction="column">
            ...
            <HStack mt="20px" spacing={3} align="center">
              {/* Filters on the left */}
              <Select value={selectedAccount} onChange={(e) => setSelectedAccount(e.target.value)} size="sm" borderRadius="md" maxW="200px" borderColor="gray.600" color="white" sx={{ "> option": { background: "#0F1535" } }}>{accounts.map((acc) => <option key={acc} value={acc}>{acc === "all" ? "All Accounts" : acc}</option>)}</Select>
              <Select value={objectiveFilter} onChange={(e) => setObjectiveFilter(e.target.value)} size="sm" borderRadius="md" maxW="200px" borderColor="gray.600" color="white" sx={{ "> option": { background: "#0F1535" } }}>{objectives.map((obj) => <option key={obj} value={obj}>{obj === "all" ? "All Objectives" : obj}</option>)}</Select>
              <Select value={datePreset} onChange={(e) => setDatePreset(e.target.value)} size="sm" borderRadius="md" maxW="150px" borderColor="gray.600" color="white" sx={{ "> option": { background: "#0F1535" } }}><option value="today">Today</option><option value="yesterday">Yesterday</option><option value="last_7d">Last 7 Days</option><option value="last_30d">Last 30 Days</option><option value="maximum">Maximum</option></Select>
              <Select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} size="sm" borderRadius="md" maxW="120px" borderColor="gray.600" color="white" sx={{ "> option": { background: "#0F1535" } }}><option value="ACTIVE">Active</option><option value="PAUSED">Paused</option><option value="ALL">All</option></Select>
              ...
              <HStack spacing={2}>
                ...
                <IconButton aria-label="Save view" icon={<Icon as={FaSave} />} size="sm" onClick={() => toast({ title: "View saved", status: "info", duration: 2000, position: "top" })} />
            ...
            <Table variant="simple" color="#fff">
              <Thead>
                <Tr my=".8rem" ps="0px">
                  ...
                  <Th color="gray.200">Actions</Th> {/* New Column Header */}
